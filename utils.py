...
def show_tsne(tsne, labels, filename, tlabels=None):
    ...
    vis_x = tsne[:, 0]
    vis_y = tsne[:, 1]
    colors = ['blue', 'orange', 'green', 'red', 'purple', 'brown', 'pink', 'yellow', 'black', 'teal', 'plum', 'tan', 'bisque', 'beige', 'slategray', 'brown', 'darkred', 'salmon', 'coral', 'olive', 'lightpink', 'darkcyan', 'darkgreen', 'darkblue', 'lightgreen', 'lightblue', 'navy', 'crimson', 'gold', 'aqua', 'orchid', 'honeydew', 'lavender', 'indianred', 'lightcoral', 'chocolate', 'wheat', 'indigo', 'lime']
...
def clustering_scores(latent, labels, cells, dataset, suffix, tlabels, louvain_num=15, prediction_algorithm="knn", X_tf=None, ensemble=False, batch_indices=None, save_cluster=False, seed=42):
    ...
    if len(labels) == 0:
        with open('result/%s/%s-cluster_result.csv'%(dataset, suffix), 'w') as f:
            f.write('cell,tlabel id,label,predicted label,tsne-1,tsne-2\n')
            for cell, pred, t in zip(cells, labels_pred, tsne):
                f.write('%s,%d,%f,%f\n'%(cell, pred, t[0], t[1]))
    else:
